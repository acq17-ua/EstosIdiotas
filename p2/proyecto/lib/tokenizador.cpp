...
bool Tokenizador::Tokenizar_ftp( const string& str, list<string>& tokens, int& i, string& curr_token ) const {
	...
	if( str.size()-i>4 ) {  // to avoid ftp: thats followed by the end of the file

		if( this->pasarAminuscSinAcentos ) {

			// comprobaci�n inicial
			if( (conversion[str[i+1]]=='t') & (conversion[str[i+2]]=='p') & (conversion[str[i+3]]==':') )	{
			
				curr_token = "ftp";
				
				c = conversion[str[i+4]];  	// despues del :
				...
				switch(c) {
					...
					case '@':
						curr_token += ':';
						curr_token += c;
						i += 5;  // despu�s de lo que acabas de leer
						break;
			...
			// comprobaci�n inicial
			if( (str[i+1]=='t') & (str[i+2]=='p') & (str[i+3]==':') )	{
			
				curr_token = "ftp";
				
				c = str[i+4];  	// despues del :
...
bool Tokenizador::Tokenizar_http( const string& str, list<string>& tokens, int& i, string& curr_token ) const {
	...
	int aux_i=i;
	char c;
	if( (str.size()-i)>5 ) {  // to avoid http: thats followed by the end of the file

		if( this->pasarAminuscSinAcentos ) {

			// comprobaci�n inicial
			if( (conversion[str[i+1]]=='t') & (conversion[str[i+2]]=='t') & (conversion[str[i+3]]=='p') )	{
				...
				else if( (str.size()-i>6) & conversion[str[i+4]]=='s' & str[i+5]==':' ) {  // https:
				...
				c = conversion[str[aux_i]];  	// despues del :
				...
				switch(c) {
					...
					case '@':
						curr_token += ':';
						curr_token += c;
						i = aux_i+1;  // despu�s de lo que acabas de leer
						break;
...
bool Tokenizador::Tokenizar_decimal( const string& str, list<string>& tokens, int& i, string& curr_token, char heading_zero ) const {
	...
	for( ; i<str.size(); i++ ) {
		...
		switch(c) {
			...
			default:
				if( this->delimitadores[c] ) {  // 123,23- � 123,23& Delimit right here
	...
	// establecer que el n�mero ya est� empezado
	for( ; i<str.size(); i++ ) {
		...
		switch(c) {
			...
			default:
				if( this->delimitadores[c] ) {  // 123,23- � 123,23& Delimit right here
					...
					i++; 		// colocar despu�s del delimitador
					if( just_dot ) { // 123.&

						tokens.push_back(string(curr_token, 0, curr_token.size()-1));
									// cambiamos el ., que escribimos por un \n, y dejamos output_size en la siguiente posici�n
						return true;
					}
...
bool Tokenizador::Tokenizar_acronimo( const string& str, list<string>& tokens, int&i, string& curr_token, bool& canBeMultiword ) const {
	...
	//cout << "final de string, meto currtoken "<< curr_token << endl; 

	if( just_dot ) // fin de string
		tokens.push_back(string(curr_token, 0, curr_token.size()-1));
...
bool Tokenizador::Tokenizar_multipalabra( const string& str, list<string>& tokens, int&i, string& curr_token ) const {
	...
	unsigned char c;
	bool just_dash = !curr_token.empty();
	i += just_dash;
...
void Tokenizador::TokenizarCasosEspeciales_UMA( const string& str, list<string>& tokens ) const {
	...
	while( i<str.size() ) {
		...
		if( Tokenizar_acronimo(str, tokens, i, curr_token, canBeMultiword) )
			continue;
...
bool Tokenizador::Tokenizar_decimal( const unsigned char* input, const size_t& input_size, unsigned char* const output, size_t& output_size, int& i, unsigned char heading_zero ) const {
	...
	// output_size es el contador de por d�nde vamos en el archivo de salida
		...
		switch(c) {
			...
			case '.':
				if( just_dot ) {   // its a normal token
					...
					i--; 			// habr� que leer el ., otra vez, por si acaso pertenece a un decimal siguiente
...
bool Tokenizador::Tokenizar_email_AM( const unsigned char* input, const size_t& input_size, unsigned char* const output, size_t& output_size, int& i, bool& canBeAcronym, bool& canBeMultiword ) const {
	...
	for( ; i<input_size; i++ ) {
		...
		switch( c ) {
			...
			case '.':  // no era un email, a�n podr�a ser un acronimo.  Multiword tendr� que comprobar si curr_token est� vac�o o no.
				output[output_size++] = c;
				canBeAcronym = true;
				return false;
			case '-':  // no era un email, a�n podr�a ser un multiword.  Multiword tendr� que comprobar si curr_token est� vac�o o no.
...
bool Tokenizador::Tokenizar_acronimo( const unsigned char* input, const size_t& input_size, unsigned char* const output, size_t& output_size, int& i, bool& canBeMultiword ) const {
	...
	for( ; i<input_size; i++ ) {
		...
		switch (c){
			...
			case '.': 			// entonces 100% es un acr�nimo, seguimos leyendo abajo
				just_dot = true;
				output[output_size++] = c;
				i++;
				goto rest;
	...
	while( i<input_size ) {
		...
		switch(c) {
			...
			case '5': case '6': case '7': case '8': case '9': 
				...
				if( Tokenizar_decimal( input, input_size, output, output_size, i, heading_dot ) ) // quitar� la coma inicial si falla
					continue;
	...
	if( !this->casosEspeciales ) {
		...
		if( !this->pasarAminuscSinAcentos ) {
			...
			for( i=0; i<input_size; i++ ) {
				...
				if( this->delimitadores[input[i]] ) { // salto de linea en el archivo de salida, sin m�s
...
// G
bool Tokenizador::PasarAminuscSinAcentos () const { return pasarAminuscSinAcentos; }
